...
function renderClaimedScores(data) {
    ...
    for (const [dimKey, dimData] of Object.entries(DIMENSIONS)) {
        ...
        const levelDesc = getLevelDescription(dimKey, claimedLevel);
...
/**
 * Generate the full L2 validated report object
...
function generateValidatedReport(validatorName, validatorRelationship, validations) {
    ...
    // Calculate score with assessment level 2 (0.8x multiplier)
    const score = calculateAIQScore(finalLevels, assesseeData.role, 2);
    ...
    const report = {
        ...
        validator: {
            name: validatorName,
            relationship: validatorRelationship
        },
        validation: {
            originalTimestamp: assesseeData.timestamp,
        ...
        notes: assesseeData.notes || undefined
    };
    ...
    for (const [dimKey, val] of Object.entries(validations)) {
        ...
        report.scores.dimensions[dimKey] = {
...
function displayResults(validations, adjustments) {
    ...
    // Display score
    document.getElementById('finalScoreValue').textContent = validatedReport.scores.finalScore;

    // Display band
    const bandBadge = document.getElementById('scoreBandBadge');
    bandBadge.textContent = validatedReport.scores.band;
    bandBadge.className = `score-band band-${validatedReport.scores.band.toLowerCase()}`;
    ...
    for (const [dimKey, dimData] of Object.entries(DIMENSIONS)) {
        ...
        const scoreData = validatedReport.scores.dimensions[dimKey];
...
/**
 * Download the validated report as PDF
 */
function downloadValidatedPDF() {
    ...
    pdfContent.innerHTML = `
        <div style="text-align: center; margin-bottom: 30px;">
            ...
            <p style="color: #64748b; font-size: 14px;">Level 2: Peer Validated</p>
            <p style="color: #64748b; font-size: 12px;">Report ID: ${validatedReport.reportId}</p>
            <p style="color: #64748b; font-size: 12px;">Generated: ${new Date(validatedReport.generatedAt).toLocaleDateString()}</p>
        </div>

        <div style="background: #f8fafc; padding: 20px; border-radius: 8px; margin-bottom: 20px;">
            <h2 style="color: #0f172a; font-size: 18px; margin-bottom: 15px;">Final Score</h2>
            <div style="text-align: center;">
                <div style="font-size: 48px; font-weight: bold; color: #4f46e5;">${validatedReport.scores.finalScore}</div>
                <div style="font-size: 16px; color: #64748b;">AIQ Score</div>
                <div style="display: inline-block; padding: 5px 15px; background: #4f46e5; color: white; border-radius: 4px; margin-top: 10px;">
                    ${validatedReport.scores.band}
                </div>
                <div style="margin-top: 10px; font-size: 12px; color: #64748b;">
                    Evidence Multiplier: 0.8x (Peer Validated)
                </div>
            </div>
        </div>
        ...
        <div style="margin-bottom: 20px;">
            ...
            <table style="width: 100%; border-collapse: collapse;">
                <tr>
                    ...
                    <td style="padding: 8px; border-bottom: 1px solid #e2e8f0;">${validatedReport.assessee.name}</td>
                </tr>
                <tr>
                    <td style="padding: 8px; border-bottom: 1px solid #e2e8f0; font-weight: 500;">Role</td>
                    <td style="padding: 8px; border-bottom: 1px solid #e2e8f0;">${validatedReport.assessee.role}</td>
                </tr>
            </table>
        </div>
        ...
        <div style="margin-bottom: 20px;">
            ...
            <table style="width: 100%; border-collapse: collapse;">
                <tr>
                    <td style="padding: 8px; border-bottom: 1px solid #e2e8f0; font-weight: 500;">Name</td>
                    <td style="padding: 8px; border-bottom: 1px solid #e2e8f0;">${validatedReport.validator.name}</td>
                </tr>
                <tr>
                    <td style="padding: 8px; border-bottom: 1px solid #e2e8f0; font-weight: 500;">Relationship</td>
                    <td style="padding: 8px; border-bottom: 1px solid #e2e8f0;">${validatedReport.validator.relationship}</td>
                </tr>
            </table>
        </div>
            ...
            <table style="width: 100%; border-collapse: collapse;">
                ...
                <tbody>
                    ${Object.entries(DIMENSIONS).map(([dimKey, dimData]) => {
                        const val = validatedReport.validation.dimensions[dimKey];
                        const scoreData = validatedReport.scores.dimensions[dimKey];
                        return `
                            <tr>
        ...
        ${Object.values(validatedReport.validation.dimensions).some(v => !v.confirmed) ? `
        <div style="margin-bottom: 20px;">
            <h2 style="color: #0f172a; font-size: 18px; margin-bottom: 15px;">Adjustments</h2>
            <ul style="margin: 0; padding-left: 20px;">
                ${Object.entries(validatedReport.validation.dimensions)
        ...
        <div style="margin-top: 30px; padding-top: 20px; border-top: 1px solid #e2e8f0; text-align: center; color: #64748b; font-size: 12px;">
            <p>Universal AIQ Framework - Level 2 Peer Validation Report</p>
            <p>Generated at ${new Date(validatedReport.generatedAt).toLocaleString()}</p>
        </div>
    `;
    ...
    const opt = {
        margin: 10,
        filename: `aiq-report-${validatedReport.reportId}.pdf`,
